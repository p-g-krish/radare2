...
// XXX. that should depend on the plugin, not the host os
			...
			//  r_arch_lazysession_set_bits (core->assembler->lsd, bits);
			// r_arch_lazysession_set_bits (core->assembler->lsa, bits);
			r_arch_lazysession_set_bits (core->anal->lsa, bits);
			//  Create a new instance if needed
			if (r_arch_lazysession_can_regprofile (core->assembler->lsd)) {
				RArchLazySession *ls = core->assembler->lsd;
